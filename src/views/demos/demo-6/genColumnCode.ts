...
export function generateColumnsCode(
  sourceCode: string,
  interfaceName: string = 'Filters',
): ColumnItemType[] {
  ...
  for (const property of myInterface.getProperties()) {
    ...
    result.push({
      ...
      key: propertyName,
      renderType: getRender(propertyName, labelName, property),
    });
...
export const generateColumnsToString = (list: ColumnItemType[]) => {
  ...
  const result = list.map(({ title, dataIndex, renderType }) => {
    return `{
      title: "${title}",
      dataIndex: "${dataIndex}",
      render: "${renderType}"
